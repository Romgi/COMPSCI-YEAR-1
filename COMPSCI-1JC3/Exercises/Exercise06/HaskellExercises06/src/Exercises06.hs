...
-- Exercise A
--------------------------------------------------------------------------------
-- Implement the Higher Order Function zipWith that behaves like zip but instead
-- of zipping a tuple, uses a binary operator provided to it as an argument.
--
-- E.x.  zipWith (*) [1,2,3] [0,0,0] = [0,0,0]
--
...
-- Exercise B
--------------------------------------------------------------------------------
-- Implement the Prelude function foldr "folds" an operator to the right. For
-- example:
--
-- E.x.  foldr (/) 1 [18,27,3] = 18 / (27 / (3 / 1))
--
-- NOTE QuickCheck test uses (-) as the function parameter
--------------------------------------------------------------------------------
foldr :: (a -> b -> b) -> b -> [a] -> b
...
-- Exercise C
--------------------------------------------------------------------------------
-- Implement the Prelude function foldl "folds" an operator to the left. For
-- example:
--
-- E.x.  foldl (/) 18 [9,2,1] = ((18 / 9) / 2) / 1
--
-- NOTE QuickCheck test uses (-) as the function parameter
--------------------------------------------------------------------------------
foldl :: (b -> a -> b) -> b -> [a] -> b
...
-- Exercise D
--------------------------------------------------------------------------------
-- Implement the Prelude function concat (which takes a list of lists, and
...
-- Exercise E
--------------------------------------------------------------------------------
-- Implement the Prelude function concatMap using foldr. concatMap takes a
-- function that returns a list, maps that function of a list then concat's the
-- result.
--
-- E.x.  concatMap (replicate 2) [1,2,3] == [1,1,2,2,3,3]
--
...
-- Exercise F
--------------------------------------------------------------------------------
-- Implement the function lookup that takes a list of tuples, where the first
-- element of the tuple serves as a key and the second element a value (a list
-- like this is also known as a dictionary), and a key value, then looks up the
-- first occuring element corresponding to that key. The return value is wrapped
-- in the Maybe type, so if the key doesn't occur anywhere in the list the
-- function returns Nothing
--
-- E.x.  lookup 2 [(0,'a'),(1,'b')] == Nothing lookup 2
--       [(0,'a'),(2,'b'),(2,'c')] == Just 'b'
--------------------------------------------------------------------------------
